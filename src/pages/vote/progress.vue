<template>
  <div class="wrap">
    ...
    <div v-if="graphic">
      <div class="bd">
        <div class="item" v-for="(item, index) in list1" :key="index">
          ...
          <!-- 通过 item.ative 实现单选或单选 -->
          <div class="title" :class="{ative:item.ative}">
            <span>{{item.title}}</span>
            <div class="progress" :class="{progressative:item.ative}">
              <div class="progress_item" :style="{width: item.num +'%'}"></div>
            </div>
            <span class="progress_num">{{item.num}}票</span>
          </div>
        </div>
        <div class="users">100人已参与投票</div>
      </div>
    </div>
    <!-- 文本投票 -->
    <div v-if="!graphic">
    ...
    <div class="btn">
      <van-button color="#D8D8D8" size="large">已投票</van-button>
    </div>
  </div>
</template>
